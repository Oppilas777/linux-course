...
Lopuksi asetin aika vyöhykkeen, jotta ohjelmat toimivat oikein, komennolla: ```timedatectl ``` ```timedatectl list-timezones``` ``` sudo timedatectl set-timezone Europe/Helsinki ```
```timedatectl ```
